...
/**
 * @ngdoc directive
 * @name rainierApp.directive:hsaArrayPolarDataVisualization
 * @description
  ...
  .directive('hsaArrayPolarDataVisualization', function (d3service, modalDialogService, synchronousTranslateService) {
    ...
    var builder = {
      ...
      _buildDonutLayout: function (d3, options) {
        ...
        _.each(sortedCircles, function(item) {
          var showOverflow = false;
          var currentValue = item.capacity ? item.capacity.value : NaN;
          ...
          if (Math.round((endAngle - angle360) * 100) / 100 > 0) {
            endAngle = angle360;
      ...
      _buildUnifiedLegendLayout: function (options, data, scope) {
        ...
        _.each(options.circles.reverse(), function (circleObject) {
          var sortedCircles = _.sortBy(circleObject.circles, function (circle) {
            // If there are no capacity in circle object, treat as smallest, used for display order only
            return circle.capacity ? circle.capacity.value : 0;
          });
      ...
      _buildBreakdownDataLayout: function (d3, data, options, scope) {
        ...
        if(data.view !== 'file' && !data.hideButtons) {
          breakdownView.append('rect')
            .on('click', function () {
              if (data.savings || data.protection) {
            ...
            .attr('fill', (data.tiers && !data.savings && !data.protection) ? '#a1e06e' : '#E3E4E7')
            .attr('stroke', 'white')
          ...
          breakdownView.append('text')
            ...
            .attr('cursor', 'pointer')
            .attr('height', bwItemHeight)
            .attr('x', bwItemLeft + switchMargin + buttonWidth + 2.5)
            .attr('font-size', 10)
            .attr('class', 'header')
            .text('Protection');
          ...
          breakdownView.append('rect')
            .on('click', function () {
              if (data.tiers || data.protection) {
                data.showSavingsBreakDown();
                scope.render(data);
              }
            })
            .attr('y', cbwy - switchTopMargin)
          ...
          breakdownView.append('text')
            ...
            .attr('cursor', 'pointer')
            .attr('height', bwItemHeight)
            .attr('x', bwItemLeft * 1.47 + switchMargin + buttonWidth + space *3)
            .attr('font-size', 10)
            .attr('class', 'header')
            .text('Savings');
        ...
        if(data.view !== 'file' && !data.tiers && !data.savings) {
          ...
          _.each(data.savingsBreakdown, function(savings) {
            ...
            breakdownView.append('text')
              .attr('y', textY)
              .attr('height', bwItemHeight)
              .attr('x', bwItemLeft + 210)
              .attr('font-size', 12)
              .text(savings.savingsRatio)
              .append('title')
              .text(savings.savingsRatio);
      ...
      buildLayout: function (d3, svg, data, hideBreakDown, hideLegendBox, onlyShowBreakDown, scope) {
        ...
        _.forEach(data.items, function (item) {
          _.forEach(item, function (line) {
            if (!_.isUndefined(line.breakdown) && !_.isNull(line.breakdown)) {
              var selected;
              if(data.tiers) {
                selected = data.tiersBreakdownLabel;
              } else if(data.protection) {
                selected = line.breakdownLabel;
              } else {
                selected = data.savingsBreakdownLabel;
              }
        ...
        var paddingBottom = Math.ceil(svgHeight/svgWidth*100) + 1;
        if (onlyShowBreakDown === true) {
          svg.attr('version', '1.1')
            .attr('viewBox', '0 0 ' + (svgWidth + 500) + ' ' + (svgHeight + space * 3))
            .attr('preserveAspectRatio', 'xMidYMin slice')
            .attr('transform', 'scale(1,1)')
            .attr('style', 'padding-bottom:' + paddingBottom + '%')
